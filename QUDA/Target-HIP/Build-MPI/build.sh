...
_main_() {
  ...
  if [ "X$2X" = 'XX' ]
  then
    echo "ERROR: Missing environment variable; 'MPI_HOME' should not be set empty! "
    return 1
  fi
  if [ "X$3X" = 'XX' ]
  then
  ...
  if [ "X$2X" = 'X/path/where/you/have/installed/mpiX' ]
  then
    echo 'MPI_HOME=/path/where/you/have/installed/mpi '
    _is_modified='N'
  fi
...
_main_ "$QUDA_HOME" "$MPI_HOME" "$EIGEN_HOME" || return
...
_MPI_PATH="$_MPI_PATH${CMAKE_PREFIX_PATH:+:}"
CMAKE_PREFIX_PATH="$_MPI_PATH$CMAKE_PREFIX_PATH"
...
cmake -Wno-dev \
 ...
 -DCMAKE_HIP_COMPILER="$(command -v dcc)" \
 -DMPI_C_COMPILER="$(command -v mpicc)" \
 -DMPI_CXX_COMPILER="$(command -v mpicxx)" \
 -DCMAKE_C_FLAGS='-O3 -fPIC' \
 -DCMAKE_CXX_FLAGS='-O3 -fPIC' \
 -DCMAKE_EXE_LINKER_FLAGS="$_LINKER" \
 -DCMAKE_SHARED_LINKER_FLAGS="$_LINKER"
